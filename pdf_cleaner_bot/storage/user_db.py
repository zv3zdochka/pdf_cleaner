...
class UserDatabase:
    ...
    def _init_db(self) -> None:
        ...
        if self.log:
            self.log.info("User database initialized at %s", self.db_path)
    ...
    def record_user_activity(
        ...
        try:
            ...
            cursor.execute("SELECT user_id, pdfs_sent, messages_count, phone_number FROM users WHERE user_id = ?",
                           (user_id,))
            row = cursor.fetchone()

            if row:
                # Update existing user
                pdfs_sent = row["pdfs_sent"] + (1 if is_pdf else 0)
            ...
            if chat_id is not None:
                cursor.execute('''
                               ...
                               VALUES (?, ?, ?, ?, ?)
                               ON CONFLICT(user_id, chat_id) DO UPDATE SET chat_type    = COALESCE(excluded.chat_type, chat_type),
